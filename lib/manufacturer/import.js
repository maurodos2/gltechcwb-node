...
const IMPORTADORES = {
  tplink: {
    marca: MARCA_TP_LINK,
    buscar: tplink.buscar,
    extrair: tplink.extrair,
  },
};

// Baixa uma imagem do host permitido e devolve { buffer, mime, nome }.
async function baixarImagem(url) {
  ...
  if (!tplink.HOSTS_IMAGENS.has(host)) return null;
...
async function importarProduto(produto, options = {}) {
  ...
  const urlProduto = await importador.buscar(modelo);
  if (!urlProduto) {
    return { status: 'nao-encontrado', motivo: `Site do fabricante não encontrou o modelo "${modelo}"` };
  }

  const dados = await importador.extrair(urlProduto);

  // Baixa imagens (principal + galeria que citam o modelo) e joga no R2
  const token = modelo.toLowerCase().replace(/[^a-z0-9]/g, '');
  const galeriaRelevante = (dados.galeria || []).filter((u) => {
    try {
  ...
  const fontes = [dados.imagemPrincipal, ...galeriaRelevante]
    .filter(Boolean)
  ...
  for (const fonte of fontes) {
    try {
      const img = await baixarImagem(fonte);
      if (!img) continue;
  ...
  if (!produto.brand) produto.brand = importador.marca;
